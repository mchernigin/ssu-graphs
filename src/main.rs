        ...
        match cmd_ans {
            ...
            "Connect two nodes" => {
                ...
                let weight = if gr.is_weighted() {
                    Some(or_escape!(CustomType::<i32>::new(
                ...
                if let Err(e) = gr.push_edge(&node1, &node2, weight) {
                    safe_err!("Cannot connect node {:?} with {:?}: {}", node1, node2, e);
                }
            }
            "Disconnect two nodes" => {
                let nodes = gr.get_nodes();
                let node1 = or_escape!(Select::new("Select first node:", nodes.clone()).prompt());
                let node2 = or_escape!(Select::new("Select second node:", nodes).prompt());
                if let Err(e) = gr.pop_edge(&node1, &node2) {
                    safe_err!("Cannot disconnect node {:?} with {:?}: {}", node1, node2, e);
                }
            }
            "Save to file" => {
